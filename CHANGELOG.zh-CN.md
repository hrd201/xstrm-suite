...
### 新增
- Web 扫描任务增加“全部文件 / 最近 24 小时新增”模式选择。
- 新增 `--recent-hours` 时间窗口扫描：刷新并遍历全部多层 AList 目录，自动包含近期新增目录下的文件，仅生成符合时间范围的视频和字幕。
- 扫描模式现在同样作用于指定目录扫描，并按创建/修改时间优先遍历较新的同级目录。
...
### 修复
- 升级时自动修复 STRM 输出目录属主；包含引号或 Shell 敏感字符的任务错误也能安全写入最终失败状态，不再残留“运行中”。
- 父目录增量扫描现在会优先处理新增、变化及从未扫描的多层子目录，保留目录元数据，并在生成新 STRM 的同时下载缺失字幕。
- 手动父目录扫描使用独立的 50 目录预算，并跳过未变化的旧子目录，使较大的新增季目录树可一次完成而不会变成全库扫描。
- AList 瞬时超时和 TLS 握手失败会先延迟重试一次，再按普通队列失败机制处理。
